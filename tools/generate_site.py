...
<div><h2>{SITE_NAME}</h2><p>Danske kalender- og hverdagsberegnere. Alt er statisk, gratis og uden login.</p></div>
...
def hero(title: str, lead: str, year: int | None = None) -> str:
    ...
    return f"""<section class="hero"><div class="container hero-grid"><div><span class="eyebrow">Dansk kalender · statisk · opdateret {ACTIVE_YEAR}</span><h1>{title}</h1><p class="lead">{lead}</p><div class="hero-actions"><a class="btn btn--primary" href="beregn-arbejdsdage.html">Beregn arbejdsdage</a><a class="btn btn--ghost" href="ugenummer.html">Find ugenummer</a></div></div><aside class="quick-panel">{side}</aside></div></section>"""
...
def render_terms() -> None:
    body = hero("Vilkår", "Betingelser for brug af DanskeDage.dk og de statiske kalenderberegnere.", date.today().year)
